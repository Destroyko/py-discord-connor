...
- [ ] **P1.1 `core/hierarchy.py`.** Чистая функция проверки: цель.top_role.position ≥ инициатор.top_role.position → отказ; цель — бот → отказ; `target.id == guild.owner_id` → отказ (явно, не полагаясь на падение API). Возвращает решение + причину; текст отказа задаёт вызывающая команда. Ref: `rules.md` § "Роли и права".
  - Проверка: `unit` (позиции, бот, owner_id — на примитивах/dataclass).
- [ ] **P1.2 `core/hierarchy.py` — self-mod.** Отдельная проверка «цель == инициатор» для `/mute`,`/ban`,`/kick` → текст `Что ты делаешь?`. Для `/unmute`,`/unban` не нужна (см. `rules.md`).
  - Проверка: `unit`.
