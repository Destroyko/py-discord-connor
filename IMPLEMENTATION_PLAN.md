...
| Член-кэш | `MemberCacheFlags(voice=True, joined=False)`, `chunk_guilds_at_startup=False` — частичный, как и было изначально. Кратко рассматривался полный кэш (`MemberCacheFlags.all()` + `chunk_guilds_at_startup=True`) специально ради `on_member_update` в вотчере `anti.py` (discord.py диспатчит это событие только участнику, уже присутствующему в кэше) — но на гильдии ~200 000 участников (см. `environment.md`) это сотни МБ RAM **постоянно**, весь аптайм, а не разовый пик при рестарте. Отклонено: вотчер `anti.py` переписан на периодический опрос `guild.audit_logs` (см. P3.2), который вообще не зависит от член-кэша. `fetch_member`-фоллбеки (P1.0d) остаются основным путём резолва цели в командах. | `environment.md` § "Технический стек" |
...
- [x] **P1.0d `fetch_member`/`fetch_user` для произвольных пользователей.** Член-кэш намеренно частичный (см. "Член-кэш" в таблице решений выше) — `guild.get_member(id)` возвращает `None` для того, кто не в войсе и не автор текущей команды, даже если он на сервере. Через REST `fetch_member`/`fetch_user` (с `except discord.NotFound`) идут: недельная перевыдача (снять роль с прежней «Души», спуск по списку победителей), резолв ников в `/ladder`, `/del` на удалённом аккаунте, резолв цели в `/ban`/`/kick`/`/mute`/`/unmute`/`/add`/`/del`/`/vkick` (`connor/core/members.py::fetch_member`). Вотчер `anti.py` (P3.2) в этом фоллбеке не нуждается вообще — он резолвит и цель, и модератора напрямую из ответа `guild.audit_logs`, member-кэша не касается. Пути редкие — рейт-лимит не проблема. **Сделано в P4:** `voices_xp.py::_resolve_member` (get_member → fetch_member) для прежней «Души» и спуска по списку; `voices_ladder.py::_name` (get_member → fetch_user → сырой id); `voices_selfmod.py::/vkick` — `guild.fetch_member` для цели.
...
- [x] **P2.B1–B4 `cogs/purge.py`.** `!purge` — обычный `commands.command` (не hybrid, не в дереве → глоб. Command-Permissions-гейт не трогает), `guild_only`.
  ...
  - **Ответ**: удаляет само сообщение-вызов (`ctx.message.delete()`, `HTTPException` глотается) — только на реальном запуске (право есть, не «роддом», разбор ок); `:pudge:` в канал вызова + embed в `#бот-команды` (None-guard, лог 1 раз): author = `global_name`+аватар; тело `<@id автора> использовал :pudge: <raw args> в канале <#канал>` (настоящее упоминание, кликабельно, не пингует — embed); число удалённых не пишется.
  - Проверка: `unit` ✓ (`tests/test_purge.py`, 29: разбор всех форм + BAD_COUNT/BAD_SYNTAX кейсы, `message_matches` все режимы, `in_roddom` канал/тред/без категории, embed, удаление сообщения-вызова на успехе / не-удаление на ошибке разбора, кликабельное упоминание в логе `#бот-команды`); smoke ✓ (ког грузится, префикс-only); `manual` — реальное удаление, граница 14 дней, лимит 300, `:pudge:`.
...
- [x] **P3.1a–c `cogs/anti.py` + `db/repo_anti.py` + `db/repo_predlozhka.py` + `predlozhka.py`.**
  ...
  - **`/add` (+`!`)**: `parse_target_id` → `ERR_NO_TARGET`; `fetch_user` NotFound → `ERR_NO_TARGET`; `repo.add` `False` → `Пользователь <@…> уже существует в списке антиработяг`. Иначе: если `member` в гильдии и `permissions_for(member).send_messages` → `apply_deny`; снять роль «работяга» (если есть). Два сообщения в `#антиработяги` (= канал вызова): embed с заголовком «Добавление» (описание, поля «Причина»/«Дата добавления» `fmt_full`, footer `Claptrap желает вам приятного дня • <fmt_full_minute>`) + embed «изъяли роль»/`Работяга` без footer'а (author = вызвавший модератор, thumbnail = аватар цели) при успехе снятия, иначе текст `Я не смог изъять роль…`.
  - **`/del` (+`!`)**: `parse_target_id` → `ERR_NO_TARGET`; `fetch_user` NotFound (удалённый аккаунт) → тихо `repo.remove` + `ERR_NO_TARGET`, без embed'ов. Не в анти-списке → `Пользователь не в списке антиработяг` (текст, не embed; роль не возвращается) **+ страховочная реконсиляция**: `clear_deny` (снимает висящий бот-овый deny в «предложке», если он остался; ручные overwrite не трогает). Иначе: `repo.remove`, `clear_deny`, **безусловно** вернуть роль (add_roles если её нет), embed с заголовком «Удаление» (без «Даты удаления») + embed «вернули роль»/`Работяга` (author = вызвавший модератор, thumbnail = аватар цели) при успехе, иначе текст `Я не смог вернуть роль так как пользователь не найден на сервере`.
  - **Гейт**: `default_permissions(moderate_members)`; `cog_command_error` missing `target` → `ERR_NO_TARGET`. `Forbidden` в рантайме → лог, тихо.
  - Проверка: `unit` ✓ (`test_repo_anti.py` 2, `test_repo_predlozhka.py` 1, `test_anti.py` 21 — embed'ы + порядок веток `/add`/`/del` вкл. предложка-deny, удалённый аккаунт, «не в списке» со снятием stale-overwrite, вотчер; на реальной БД + фейках Discord); smoke ✓.
  - **wiki:** `anti.md` дописан — `/del` на «не в списке» (текст `Пользователь не в списке антиработяг` + снятие stale bot-overwrite) в §"Логика работы" п.5 и таблице ошибок; заголовки embed'ов «Добавление»/«Удаление», author (инициатор) + thumbnail (аватар цели) на embed'ах «изъяли/вернули роль»; цвета embed'ов anti (красный add/изъятие, зелёный del/возврат) — спека их по-прежнему не задаёт, оставлено на усмотрение реализации.

### P3.2 anti — вотчер ручных изменений роли — `anti.py` · Ref: `anti.md` § "Наблюдение…"

- [x] **P3.2a–b Watcher в `cogs/anti.py`: `tasks.loop(seconds=3)` опроса audit log** (не `on_member_update` — см. "Член-кэш" в таблице решений выше). Интервал короче задержки появления записи в API (~4–5 c) — короче уже бессмысленно, запись раньше не появится; реальная задержка реакции ≈5–8 c от события. `cog_load`/`cog_unload` — старт/остановка цикла; `before_loop` ждёт `wait_until_ready`; `@loop.error` — `log.exception` (P1.0a). Тело (`_poll_once`) в `try/except` внутри самого цикла — сбой одной итерации не останавливает цикл.
  - **Курсор** (`db/repo_anti_watcher.py::RepoAntiWatcher`, таблица `anti_watcher_cursor`, миграция `0003`) — id последней обработанной записи аудита, персистентен. Первый чистый старт (курсор `None`) — не разбираем историю, только запоминаем id самой свежей на тот момент записи (`guild.audit_logs(limit=1)`), аналогично `anchor_ts` в `voice_cycle`.
  - **Опрос**: `guild.audit_logs(action=member_role_update, after=Object(id=cursor), oldest_first=True, limit=None)` — по каждой записи проверяем роль «работяга» в `entry.before.roles` (снята) / `entry.after.roles` (выдана; оба независимо, не `elif`), курсор двигаем на `entry.id` после каждой записи (даже нерелевантной/от бота) — не переобрабатывать при следующем сбое. Автор == `bot.user_id` → пропуск (команда уже отчиталась); автор `None` → `log.warning`, пропуск. Цель и модератор резолвятся из `entry.target`/`entry.user` — они приходят из данных самого ответа audit log API, не из member-кэша (см. `_get_member` в `discord.py`: `guild.get_member(...) or self._users.get(...)` — второе не зависит от кэша).
  - **Снятие вручную** → `#антиработяги`: `build_role_removed_embed(mention, moderator=actor, target_avatar_url=...)` — тот же формат, что у `/add`; независимо от анти-статуса. `mention`/`avatar_url` строятся из `entry.target.id`/`.display_avatar` без похода за `Member` — работают даже если резолв дал только `User`, не полноценного `Member`.
  - **Выдача вручную анти-работяге** (`anti_repo.contains`) → `anti_repo.remove` + `clear_deny` в «предложке» (только если `entry.target` — реальный `User`/`Member`, не голый `Object`; `set_permissions` требует именно это) + `#антиработяги`: embed «Удаление» (причина `роль «работяга» выдана вручную`) + `build_role_returned_embed(mention, moderator=actor, target_avatar_url=...)`. Не в списке → тихо.
  - Проверка: `unit` ✓ (`test_anti.py` — первый старт сеет курсор без обработки, пустая история не двигает курсор, нерелевантная смена роли двигает курсор без уведомления, снятие модератором/ботом, выдача анти/не-анти, актор не найден; `test_repo_anti_watcher.py` — курсор get/set/overwrite); smoke ✓.
...
- [x] **P3.4a–f `cogs/role_giver.py` + `db/repo_give.py`.**
  ...
  - **`build_audit_embed`**: одобрено → зелёный, описание `<@mod> обновил <@target>` (настоящие упоминания, кликабельны, не пингуют — embed), поле `Выдана роль` → `<@&Работяга>`, thumbnail = аватар цели (серверный/фоллбек глобальный); отказано → красный, `<@mod> отказал <@target>` / поле `Отказ в выдаче роли`. Сц.1 и 3 не логируются.
...
- [x] **P4.3a `/vkick` (+`!`) в `#флудиславль`.** Проверка: вызвавший — владелец **бот-комнаты** из реестра и сейчас в ней (иначе `У вас нет прав… или вы не находитесь в голосовом канале`). На общие/предустановленные войсы не распространяется. Цель: обязана быть на сервере (`Вы не указали пользователя или его нет на сервере`), присутствие в войсе не требуется. Самомодерация: цель = вызывающий → `Вы не можете выгнать самого себя` (до `fetch_member`). Действия: disconnect **только** если цель в комнате вызвавшего (в другом канале — не трогать); в любом случае — deny `Connect` overwrite на комнату + снять доступ к встроенному текст-чату + запись в `voice_banlist` (`ts`=now). Лимит **100** → `Список забаненных пользователей достиг лимита. Используйте команду banList для подробностей`. Успех → `Пользователь @… забанен в ваших комнатах.` Ответ ephemeral для slash, реплаем на `!vkick`. Описание Discord: `Кикнуть %username% из приватного войс канала`.
  - **Сделано:** `db/repo_voice_banlist.py` (`RepoVoiceBanlist`: `upsert`/`remove`/`contains`/`count`/`list_for`/`active_ids`). `/vkick` hybrid, без `default_permissions`, `guild_only`; `is_self_moderation` (`core/hierarchy.py`) режет цель=автора раньше `fetch_member`; цель через `guild.fetch_member` (P1.0d); overwrite `connect=False` + `send_messages=False` (встроенный текст-чат; видимость канала не трогаем); disconnect только при совпадении канала; лимит блокирует лишь **новую** запись (повторный `/vkick` по уже забаненному проходит). Ответ `ephemeral=True` (для `!` игнорируется Discord).
...
- [x] **P4.3b `/vreturn` (+`!`) / `!vdel` (ЛС).** Без проверки владения/присутствия. Убрать из `voice_banlist`; если у вызвавшего есть активная комната и на ней стоит deny `Connect` для этого id — **сразу снять**. Успех → `Пользователь @… разбанен.`; нет в списке → `Пользователь не найден в списке забаненных.` `%username%`-описания зафиксированы в вики. `/vreturn`/`!vreturn` — ephemeral для slash, реплаем на `!`; `!vdel` (только ЛС) — обычным сообщением, без изменений.
...
- [x] **P4.3d `/ban_list` / `!ban_list`.** Всегда шлёт список в ЛС (embed: синяя полоса, заголовок `Список забаненных`, описание с `!vreturn`/`!vdel`, записи с кликабельным `<@id>` (не в code-разметке — резолвится даже для тех, кто не на общем сервере) + `id: <id>`, footer-пример). В месте вызова: `Информация отправлена в лс` / `Я не могу отправить тебе список. Открой ЛС`. `!ban_list` доступен и в ЛС боту. Описание Discord: `Управление бан-списком приваток`.
